...
sets.craftgear = {ring1="Craftmaster\'s Ring",ring2="Craftkeeper\'s Ring"}
...
ShowStatus_txt = {}
ShowStatus_txt.pos = {}
ShowStatus_txt.pos.x = 575
--ShowStatus_txt.pos.y = 715
ShowStatus_txt.pos.y = 880
ShowStatus_txt.text = {}
ShowStatus_txt.text.font = 'Arial'
ShowStatus_txt.text.size = 10
...
function checkblocking(spell)
	    ...
	    if buffactive.Silence then
          ...
          if spell and (recasttime >= 1) then
			if spell.skill == 'Elemental Magic' and AutoNextTier then 
				...
				add_to_chat(210,'Spell Canceled:'..spell.name..' - Waiting on Recast:(seconds) '..recasttime..'')
				cancel_spell()
				send_command('gs c update')
				return
			end
    ...
    if spell.type == 'WeaponSkill' and player.tp < 1000 then
		cancel_spell()
		send_command('gs c update')
		add_to_chat(209,'Canceled WS: '..spell.name..' [ Waiting on TP ]')
		return
	end
	if spell.type == 'WeaponSkill' and buffactive.Amnesia then
		  cancel_spell()
		  send_command('gs c update')
		  add_to_chat(29,'Canceling Ability - Currently have Amnesia')
		  return	  
	end
	if spell.type == 'WeaponSkill' then
	    local range_mult = {
		...
		if spell.target.distance > (ability_distance * range_mult[ability_distance] + spell.target.model_size + player.model_size) then
		    cancel_spell()
			add_to_chat(29, spell.name..' Canceled WS: [Out of Range]')
			send_command('gs c update')
			return
		end
    end
